...
%$ Company value (today) = Sum(cash_flow_i) for i = 1 to infinity $
\begin{equation}
CompanyValue_{today} = \sum_i^{\inf} CashFlow_i
\end{equation}
...
\begin{equation}
CompanyValue_{today} = \sum_i^{\inf} \frac{CashFlow_i}{1 + WACC^i}
\end{equation}
...
\begin{equation}
CompanyValue_{today} = \sum_i^{10} CashFlow_{today} * (\frac{1 + g_1}{(1 + WACC)})^i + \sum_11^{\inf} CashFlow_year10 * (\frac{1 + g_2}{1 + WACC})^i
\end{equation}
...
\begin{equation}
CompanyValue_{today} = \sum_i^{10} CashFlow_{today} * (\frac{1 + g_1}{(1 + WACC)})^i + CashFlow_year10 * \frac{1}{WACC - g2}
\end{equation}
